const gradientMapping = {
  ...
  gray: 'linear-gradient(hsl(0, 0%, 22%), hsl(0, 0%, 12%))',
};
...
const GlassIcons = ({ items, className }) => {
  ...
  return (
    <div
      className={`grid gap-[4em] sm:gap-[5em] grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 mx-auto py-[2em] sm:py-[3em] overflow-visible ${className || ''}`}>
      {items.map((item, index) => (
        <button
          ...
          <span
            ...
            <span
              className="m-auto w-[1.5em] h-[1.5em] flex items-center justify-center text-gray-200"
              aria-hidden="true">
              {item.icon}
            </span>
          </span>
          ...
          <span
            className="absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] text-xs sm:text-sm text-gray-300 opacity-100 sm:opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-1 sm:translate-y-0 sm:group-hover:opacity-100 sm:group-hover:[transform:translateY(20%)]">
            {item.label}
          </span>
        </button>
      ))}
